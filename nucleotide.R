...
## The sorted lists in "nucleotide_output" can then be uploaded into InteractiVenn to generate comparative diagrams.
...
for(nuc_file in nuc_files){
  ...
  # Save edited list for Interactivenn use-----------------------------------------------------
  temp_outName = paste('OUTPUT/nucleotide_output/', gene,'.csv', sep='', collapse = '')
  write.table(nuc_df, temp_outName, col.names = F, row.names = F, sep = ",")
}

# Load edited nucleotide input files-----------------------------------------------------------
nuc_tempdir = "OUTPUT/nucleotide_output"
...
out_Table = paste('OUTPUT/', "nucleotide_table",'.csv', sep='', collapse = '')
